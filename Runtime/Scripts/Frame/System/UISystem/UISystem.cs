...
namespace Cheems.UI
{
    ...
    /// UI根节点
    /// </summary>
    public partial class UISystem : PersistentMonoSingleton<UISystem>
    {
        #region Signleton方法

        protected override void OnInitialized()
        {
            // CLog.Warning("UI系统初始化");
            base.OnInitialized();
            UIWindowDataDic = GameConfig.UIConfig.GetDataDic();
        }
        ...
        private class UILayer
        {
            ...
            private void UpdateMask()
            {
                ...
                int posIndex = root.childCount - 1;
        ...
        private static Dictionary<string, UIWindowData> UIWindowDataDic;
        ...
        /// 拖拽层，位于所有UI的最上层
        /// </summary>
        public static RectTransform DragLayer
        {
            get => Instance.dragLayer;
        }

        private static UILayer[] UILayers
        {
            get => Instance.uiLayers;
        }

        public static RectTransform MainCanvasRect
        {
            get => Instance.transform as RectTransform;
        }
        ...
        public static Canvas MainCanvas
        {
            get
            {
                if (Instance._mainCanvas == null)
                {
                    Instance._mainCanvas = Instance.GetComponent<Canvas>();
                }

                return Instance._mainCanvas;
            }
        }
        ...
        public static void AddUIWindowData(string windowKey, UIWindowData windowData, bool instantiateAtOnce = false)
        {
            if (UIWindowDataDic.TryAdd(windowKey, windowData))
            {
                if (instantiateAtOnce)
                {
                    ...
                    {
                        UIWindowBase window =
                            ResSystem.InstantiateGameObject<UIWindowBase>(
                                windowData.assetPath, UILayers[windowData.layerNum].root, windowKey);
        ...
        public static T Show<T>(int layer = -1) where T : UIWindowBase
        {
        ...
        /// <typeparam name="T">窗口类型</typeparam>
        /// <param name="layer">层级 -1等于不设置</param>
        public static void ShowAsync<T>(Action<T> callback = null, int layer = -1) where T : UIWindowBase
        {
        ...
        /// <param name="layer">层级 -1等于不设置</param>
        public static T Show<T>(string windowKey, int layer = -1) where T : UIWindowBase
        {
        ...
        /// <param name="layer">层级 -1等于不设置</param>
        public static UIWindowBase Show(Type type, int layer = -1)
        {
        ...
        /// <param name="type">窗口类型</param>
        /// <param name="layer">层级 -1等于不设置</param>
        public static void ShowAsync(Type type, Action<UIWindowBase> callback = null, int layer = -1)
        {
        ...
        /// <param name="windowKey">窗口的key</param>
        /// <param name="layer">层级 -1等于不设置</param>
        public static UIWindowBase Show(string windowKey, int layer = -1)
        {
            if (UIWindowDataDic.TryGetValue(windowKey, out UIWindowData windowData))
            {
            ...
            // 资源库中没有意味着不允许显示
            Debug.Log($"不存在{windowKey}的UIWindowData");
            return null;
        }
        ...
        /// <param name="windowKey">窗口的key</param>
        /// <param name="layer">层级 -1等于不设置</param>
        public static void ShowAsync(string windowKey, Action<UIWindowBase> callback = null, int layer = -1)
        {
            if (UIWindowDataDic.TryGetValue(windowKey, out UIWindowData windowData))
            {
                ShowAsync(windowData, windowKey, callback, layer);
            }
            else Debug.Log($"不存在{windowKey}的UIWindowData"); // 资源库中没有意味着不允许显示
        }

        private static UIWindowBase Show(UIWindowData windowData, string windowKey, int layer = -1)
        {
            ...
            if (windowData.instance != null)
            {
                ...
                if (windowData.instance.IsUIEnable)
                {
            ...
            else
            {
                UIWindowBase window =
                    ResSystem.InstantiateGameObject<UIWindowBase>(
                        windowData.assetPath, UILayers[layerNum].root, windowKey);
        ...
        private static void ShowAsync(UIWindowData windowData, string windowKey, Action<UIWindowBase> callback = null,
                                      int layer = -1)
        {
            int layerNum = layer == -1 ? windowData.layerNum : layer;
            // 实例化实例或者获取到实例，保证窗口实例存在
            ...
            {
                ResSystem.InstantiateGameObjectAsync<UIWindowBase>(windowData.assetPath,
                                                                   (window) =>
                                                                   {
                                                                       windowData.instance = window;
                                                                       window.Init();
        ...
        public static UIWindowBase GetWindow(string windowKey)
        {
            if (UIWindowDataDic.TryGetValue(windowKey, out UIWindowData windowData))
            {
                return windowData.instance;
        ...
        /// <param name="windowKey">窗口Key</param>
        /// <returns>没找到会为Null</returns>
        public static T GetWindow<T>(string windowKey) where T : UIWindowBase
        {
        ...
        /// <param name="windowKey">窗口Key</param>
        /// <returns>没找到会为Null</returns>
        public static T GetWindow<T>(Type windowType) where T : UIWindowBase
        {
        ...
        /// </summary>
        /// <param name="windowKey"></param>
        public static bool TryGetWindow(string windowKey, out UIWindowBase window)
        {
        ...
        /// </summary>
        /// <param name="windowKey"></param>
        public static bool TryGetWindow<T>(string windowKey, out T window) where T : UIWindowBase
        {
            UIWindowDataDic.TryGetValue(windowKey, out UIWindowData windowData);
        ...
        {
            UIWindowBase window = GetWindow(windowKey);
            if (window != null)
            {
        ...
        /// </summary>
        /// <typeparam name="T">窗口类型</typeparam>
        /// <param name="isHide">决定UI面板是否关闭，只对isCache的GameObject有用</param>
        public static void Close<T>(bool isHide = false)
        {
            Close(typeof(T), isHide);
        }
        ...
        /// <param name="isHide">决定UI面板是否关闭，只对isCache的GameObject有用</param>
        public static void Close(Type type, bool isHide = false)
        {
            Close(type.FullName, isHide);
        }
        ...
        /// <param name="windowKey"></param>
        /// /// <param name="isHide">决定UI面板是否关闭，只对isCache的GameObject有用</param>
        public static void Close(string windowKey, bool isHide = false)
        {
            if (TryGetUIWindowData(windowKey, out UIWindowData windowData))
            {
                if (windowData.instance != null && CloseWindow(windowData, isHide))
                {
                    UILayers[windowData.layerNum].OnWindowClose();
                }
                // else Debug.LogWarning("您需要关闭的窗口不存在或已经关闭");
            }
            else Debug.LogWarning("未查询到UIWindowData");
        }
        ...
        /// <typeparam name="T">窗口类型</typeparam>
        /// <param name="isHide">决定UI面板是否关闭，只对isCache的GameObject有用</param>
        public static void TryClose<T>(bool isHide = false)
        {
            TryClose(typeof(T), isHide);
        }
        ...
        /// <param name="type">窗口类型</param>
        /// <param name="isHide">决定UI面板是否关闭，只对isCache的GameObject有用</param>
        public static void TryClose(Type type, bool isHide = false)
        {
            TryClose(type.FullName, isHide);
        }

        /// <summary>
        /// 尝试关闭窗口
        /// </summary>
        public static bool TryClose(string windowKey, bool isHide = false)
        {
            if (TryGetUIWindowData(windowKey, out UIWindowData windowData))
            {
                if (windowData.instance != null && CloseWindow(windowData, isHide))
        ...
        private static bool CloseWindow(UIWindowData windowData, bool isHide = false)
        {
            if (windowData.instance.IsUIEnable)
            {
                ...
                if (windowData.isCache)
                {
                    ...
                    windowData.instance.gameObject.SetActive(!isHide);
                }
                // 不缓存则销毁
                else
                {
#if ENABLE_ADDRESSABLES
                    ResourcesManager.UnloadInstance(windowData.instance.gameObject);
#endif
        ...
        /// </summary>
        /// <returns></returns>
        public static Vector2 GetUIFollowMousePos()
        {
        ...
        public static bool CheckMouseOnUI()
        {
            ...
            return CheckPositionOnUI(UnityEngine.Input.mousePosition);
#else
