...
namespace Cheems
{
    public class ObjectPoolModule
    {
        ...
        /// 普通类 对象容器
        /// </summary>
        public readonly Dictionary<string, ObjectPoolData> poolDic = new();
        ...
        public ObjectPoolData ClearObject<T>()
        {
           return ClearObject(typeof(T).FullName);
        }

        public ObjectPoolData ClearObject(System.Type type)
        {
           return ClearObject(type.FullName);
        }

        public ObjectPoolData ClearObject(string keyName)
        {
            ObjectPoolData data = null;
            if (poolDic.TryGetValue(keyName, out ObjectPoolData objectPoolData))
            {
                data = objectPoolData.Destroy(true);
